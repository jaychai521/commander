...
let WxWebRequestLogin = cc.Class({
    ...
    authCommit() {
        ...
        if(GlobalData.User.Is_Auth == 0){   //如果用户没有授过权
            wx.request({
                url: GlobalData.TrafficUrlTest1 + GlobalData.Update,
                method: "post",
                data: {
                    type: 2,
                    from:this.from
    ...
    firstLogin(){
        ...
        if(GlobalData.User.First_Login == 0) {
            wx.request({
                url: GlobalData.TrafficUrlTest + GlobalData.Update,
                method: "post",
                data: {
                    type: 1,
                    from: this.from
    ...
    onShareAppMessage() {
        if (this.isRunWeChat()) {
            ...
            let defaultShareTitle = "9012年最强升官游戏 不想当司令的兵不是好兵!";
    ...
    //发送用户code给服务器（登录）
    wx_login(code, userInfo,autoAuth=true) {
            ...
            wx.request({
                ...
                success(res) {
                    ...
                    if (res.data.data) {
                        ...
                        GlobalData.User.YBCount = data.love;
                        GlobalData.User.CoinCount = data.rice;
                        GlobalData.User.Comds = data.chook;
                        GlobalData.User.Level_YB = data.level_love;
                        GlobalData.User.Level_Coin = data.level_rice;
                        GlobalData.User.Fly_Level = data.fly_level;
                        GlobalData.User.Comd_level = data.chook_level;
                        ...
                        GlobalData.User.Fast_YB = data.fast_love;
            ...
            videoAd.load()
                ...
                .catch(err => {
                    ...
                    if (!err.errCode) {
                        ...
                        switch (from) {
                            case 'comment_double':
                                cc.director.GlobalEvent.emit('comment_double_share_Comd',{eventName:''});
                                break;
                            case 'extraReward_receive':
                                cc.director.GlobalEvent.emit('extraReward_receive_share_Comd',{eventName:''});
                                break;
                            case 'secretReward_receive':
                                cc.director.GlobalEvent.emit('secretReward_receive_share_Comd',{eventName:''});
                                break;
                            case 'offlineReward_double':
                                cc.director.GlobalEvent.emit('offlineReward_double_share_Comd',{eventName:''});
                                break;
                            case 'onLuckyLevelUp':
                                cc.director.GlobalEvent.emit('onLuckyUp_Share_Comd',{eventName:''});
                                break;
        ...
        if (this.isRunWeChat()) {
            wx.request({
                ...
                success(res) {
                    ...
                    let data = res.data.data
                    console.log("get limitNum success", checkType, data);
                    //today_times:今天已看次数. can: 1表示今天还可以签到/看视频/抽奖，0不能
                    if (data) {
                        if (checkType === "video") {
                            GlobalData.VideoLimitAry.todayTime = data.today_times;
                            GlobalData.VideoLimitAry.isCan = data.can;
                            GlobalData.VideoLimitAry.lastTime = GlobalData.VideoLimitAry.allNum - GlobalData.VideoLimitAry.todayTime;
                            // console.log("GlobalData.VideoLimitAry:", GlobalData.VideoLimitAry.lastTime);
                        }
                        else if (checkType === "share") {
                            GlobalData.ShareLimitAry.todayTime = data.today_times;
                            GlobalData.ShareLimitAry.isCan = data.can;
                            GlobalData.ShareLimitAry.lastTime = GlobalData.ShareLimitAry.allNum - GlobalData.ShareLimitAry.todayTime;
                        }
                        else if (checkType === "lucky") {
                            GlobalData.LuckyLimitAry.todayTime = data.today_times;
                            GlobalData.LuckyLimitAry.isCan = data.can;
                            GlobalData.LuckyLimitAry.lastTime = GlobalData.LuckyLimitAry.allNum - GlobalData.LuckyLimitAry.todayTime;
                        }

                    }
                },
                fail() {
                    console.log("get limitNum fail");
                }
            });
        }
    },
            ...
            wx.request({
                ...
                success(res) {
                    ...
                    if (data) {
                        ...
                        GlobalData.User.Fast_YB = data.fast_yb; //更新加速次数
                    }
                    else {
                        cc.log('data is null...');
                    }
    ...
    onShow(){
        ...
        wx.onShow(function (res) {
            ...
            if(GlobalData.OnShowTypeValue === GlobalData.OnShowType.OFFLINE_DOUBLE_REWARD){
                cc.director.GlobalEvent.emit('offlineReward_reward_Comd',{eventName:'',result: bSuccess});
            }
            else  if(GlobalData.OnShowTypeValue === GlobalData.OnShowType.SECRET_REWARD){
                cc.director.GlobalEvent.emit('secretReward_reward_Comd',{eventName:'',result: bSuccess});
                console.log('onSecretReward');
            }
            else if(GlobalData.OnShowTypeValue === GlobalData.OnShowType.COMMENT_DOUBLE_LOVE){
                cc.director.GlobalEvent.emit('comment_reward_Comd',{eventName:'',result: bSuccess});
            }
            else if(GlobalData.OnShowTypeValue === GlobalData.OnShowType.EXTRA_REWARD){
                cc.director.GlobalEvent.emit('extraReward_reward_Comd',{eventName:'',result: bSuccess});
            }
            else if(GlobalData.OnShowTypeValue === GlobalData.OnShowType.LUCKY_LEVELUP){
                cc.director.GlobalEvent.emit('onLuckyLevelUp_Share_Comd',{eventName:'',result: bSuccess});
            }
            else if(GlobalData.OnShowTypeValue === GlobalData.OnShowType.REDPACKAGE_RAIN_REWARD){
                cc.director.GlobalEvent.emit('rain_reward_Comd',{eventName:'',result: bSuccess});
            }
            else if(GlobalData.OnShowTypeValue === GlobalData.OnShowType.SHOW_OFF){
                cc.director.GlobalEvent.emit('showOffReward_Comd',{eventName:'',result: bSuccess});
            }
    ...
    CheckLaunchData(type,autoAuth) {
        ...
        if (!querydata) {
            ...
            if(this.LaunchConfig.query.from){
                ...
                this.firstLogin();  //首次登陆，进行流量统计
                if(!autoAuth)       //如果是通过点击授权按钮进来的玩家，进行授权流量统计
                    this.authCommit();
            }
