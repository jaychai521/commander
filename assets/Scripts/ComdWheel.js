...
let ComdRewardController = require("ComdRewardController");
...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.spinBtn.node.on(cc.Node.EventType.TOUCH_END,function(event)
        {
                ...
                let bRes = GoldSystem._instance.getleavesResult();
                if(!bRes){
                    ComdUIManager._instance.showWheelWarmTip_Comd(true);
                    return;
                }
                cc.log("begin spin");

                ComdCommandData._instance.UpdateLuckyWhell();   //提交数据
                 GoldSystem._instance.LeavesOperate(-1);
                ...
                this.targetID = ComdRewardController._instance.getSpinTargetID();

                this.decAngle = 2*360;  // 减速旋转两圈
                this.wheelState = 1;
                this.curSpeed = 0;
                this.spinTime = 0;
                // let act = cc.rotateTo(10, 360*10);
        ...
        else if(this.wheelState == 2)
        {
          //   cc.log('......减速');
            ...
            //     cc.log('....停止');
                ...
                {
                    //倒转一个齿轮
                    // let act = new cc.rotateBy(0.6, -this.gearAngle);
    ...
    showRes:function()
    {
        ComdRewardController._instance.getReward(this.targetID);
