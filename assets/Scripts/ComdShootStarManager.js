...
cc.Class({
    ...
    addRedpackageRainScore(event){
        let res = GoldSystem._instance.getRainResult(event.type,event.score);
        //米粒
        if(event.type == 1){
            this._coinScore+=res[0];
            //cc.log('+res:',res[0]);

            let str = this._coinScore.toString();
            let index = str.indexOf('.');
            if(index !== -1){
                str = str.substring(0,index + 3);
            }
            this.scoreCoinLabel.string = str + res[1];
            cc.director.GlobalEvent.emit('updateRaindReward',{type:event.type,data:[this._coinScore,res[1]]});
        }
        //爱心
        if(event.type == 2){
            this._ybScore+=res;
            this.scoreYBLabel.string = this._ybScore;
            cc.director.GlobalEvent.emit('updateRaindReward',{type:event.type,data:this._ybScore});
        }
    ...
    startRain(){
        ...
        this.scoreCoinLabel.string = '0';
        this.scoreYBLabel.string = '0';
    ...
    spawnStar(bCoin){
        if(bCoin){
            let inx = parseInt(Math.random()*10)%this.coinPrefabs.length;
            let obj =  cc.instantiate(this.coinPrefabs[inx]);
            obj.parent = this.starNode;
        }
        else {
            let inx = parseInt(Math.random()*10)%this.ybPrefabs.length;
            let obj =  cc.instantiate(this.ybPrefabs[inx]);
            obj.parent = this.starNode;
        }
    },
    ...
    update (dt) {
        ...
        else
        {
            ...
            //红包雨的产生速率
            this.spawnFactor = Math.sin((25-this._timer)/9);
            this.spawnInterval +=dt*(0.5+this.spawnFactor*2);
            if(this.spawnInterval >= 0.4){
                this.spawnInterval = 0;
                //TO DO LIST
                let bMore = GoldSystem._instance.compareCoins();
                let bCoin = this.rollPoint(bMore);
                this.spawnStar(bCoin);
            }
        }
