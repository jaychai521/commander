...
cc.Class({
    ...
    onClick(){
        GoldSystem._instance.YBOperate(this.rewardYB,{tip:'章+',content:this.rewardYB});
        ComdUIManager._instance.showExtraReward_Comd(true);
        //通知数据管理器进行今日签到
    ...
    updateSignStatus(){
        let self = this;
        let _maxDay = ComdCommandData._instance.MaxSign;
        ...
        if(self.signID <= _maxDay && _maxDay > 0){
            ...
            console.log('今日第',self.signID,'天签到',' 签到状态:',ComdCommandData._instance.TodaySign);
            let bSign = (ComdCommandData._instance.TodaySign === 1) ? true:false;
            if(bSign){
