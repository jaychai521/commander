...
cc.director.GuideManager = {
    ...
    startCurGuide(id){
        this._guideIds = ComdCommandData._instance.Tutorials;
        if(this._guideIds[id] == 1)
            return;
        this._guideIds[id] = 1;
        ComdCommandData._instance.Tutorials = this._guideIds;
        let path = GlobalData.GuideInfo[id].path;
        let parent = GlobalData.GuideInfo[id].parent;
        ComdUIManager._instance.showGuideOper_Comd(path,parent);
    },
